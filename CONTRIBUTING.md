# Contributing

ReviewInbox is a pnpm and Nx monorepo. Keep package boundaries explicit: `apps/web` is the Angular product app, `apps/api` is the Hono backend, `apps/worker` owns background jobs, and shared code lives under `packages/*`.

## Local Setup

Use `mise install` to install the Node.js version pinned in `mise.toml`. Activate mise in your shell, or prefix commands with `mise exec --`. The pnpm version is pinned by the root `package.json` `packageManager` field; install that exact version before running workspace commands:

```bash
npm install --global "$(node --print 'require("./package.json").packageManager')"
```

Install dependencies:

```bash
pnpm install
```

Create a local environment file:

```bash
cp .env.example .env
```

Set local secrets in `.env`:

```bash
APP_ENCRYPTION_KEY=$(openssl rand -base64 32)
BETTER_AUTH_SECRET=$(openssl rand -base64 32)
```

Keep `BETTER_AUTH_URL` aligned with the Hono API origin. For local development it defaults to `http://127.0.0.1:3000`, while `BETTER_AUTH_TRUSTED_ORIGINS` includes the Angular dev server origins used by the `/api` proxy.

AI drafting is disabled by default. To test autogenerated Reply Drafts locally, set `REPLY_DRAFT_WORKER_ENABLED=true` for the API and set `AI_PROVIDER=openai-compatible`, `AI_MODEL`, and `AI_API_KEY` for the worker. When Review Analysis is enabled, provide the same `AI_*` values to the API so it can report topic-discovery availability.

Review analysis uses the optional global `TYPESAFE_API_KEY` on the API and worker. Jev classification covers imported Reviews and the full history backfill independently of Managed AI Reply Draft quotas. Luna topic discovery reuses the configured generative provider settings (`AI_PROVIDER`, `AI_MODEL`, `AI_API_KEY`, and optional `AI_BASE_URL`) and runs in bounded, separately metered batches. If `TYPESAFE_API_KEY` is absent, stored analysis remains readable while new analysis jobs stay unavailable; review sync and Reply Draft generation are unaffected.

Apple Review versions are enriched automatically by the worker at startup and after ingestion. The worker groups pending Reviews by Store Connection and traverses Apple's iOS version-scoped review pages, stopping once all target IDs are resolved. No version catalogue table is maintained. Reviews and replies remain available during lookup; temporary failures keep the lookup pending, while a fully exhausted search marks the version unavailable. Edited Reviews are checked again without letting a late result overwrite their new content.

The Store Connection stores only an in-progress pagination cursor. Completed pages and their matches are committed together, so a restart or rate-limit retry resumes without repeating completed pages. Searches sharing an Apple credential are serialized, and enrichment pauses while ingestion using the same credential is active in the worker. Enrichment leaves 10% of the advertised hourly quota for normal ingestion. On `429`, it respects `Retry-After` when present, otherwise waits an hour; other failures retry after five minutes. A worker sweep checks pending work every minute. No additional environment variable is required.

Start Postgres:

```bash
docker compose up -d postgres
```

Apply database migrations:

```bash
pnpm db:migrate
```

Alternatively, set `RUN_DB_MIGRATIONS_ON_STARTUP=true` to let the API apply pending migrations before it starts. Keep this disabled for multi-instance deployments unless migrations are run by a single deployment job.

Start the product app stack:

```bash
pnpm nx run-many -t serve -p api web
```

Open `http://localhost:4200`. The Angular dev server proxies `/api` paths, including `/api/auth`, to Hono on `http://127.0.0.1:3000`.

## Checks

Before committing, run:

```bash
pnpm format
pnpm format:check
pnpm lint
pnpm typecheck
pnpm test
pnpm build
```

Nx owns task orchestration for the monorepo. Prefer `pnpm nx affected -t <target>` for focused checks once the workspace is initialized.
Oxfmt owns formatting (`.oxfmtrc.json`); Oxlint owns linting (`oxlint.config.ts`), including type-aware, SonarJS, and anti-slop rules. Run `pnpm lint:fix` for automatic lint fixes followed by formatting. Markdown and generated database migrations are excluded from formatting.

## Docker Images

Self-hosted deployments use three app images plus Postgres:

- `ghcr.io/bibliocards/reviewinbox-api:<semver>` from `apps/api/Dockerfile`
- `ghcr.io/bibliocards/reviewinbox-web:<semver>` from `apps/web/Dockerfile`
- `ghcr.io/bibliocards/reviewinbox-worker:<semver>` from `apps/worker/Dockerfile`

The self-hosted compose file pins images through `REVIEWINBOX_VERSION` in `.env.self-hosted`. Do not document or recommend `latest` for VPS installs.

Image publishing CI is intentionally separate from this local development flow and should tag all three images with the same semver release.

## Database

Generate migrations after schema changes:

```bash
pnpm db:generate --name add_review_field
```

Migrations live in `packages/db/migrations`. Like Hermes, this repository keeps the complete SQL history and journal, but only the latest schema snapshot at `meta/snapshot.json`.

`pnpm db:generate` runs Drizzle Kit in a temporary directory, validates that generation only appends to the journal, then publishes the new SQL, journal, and snapshot together with rollback on publication failure. An unchanged schema leaves these files untouched. Commit all three generated files with the schema change. Use this wrapper instead of calling `drizzle-kit generate` directly; `--out`, `--config`, and `--prefix` are managed by the wrapper.

Existing SQL migrations and their journal entries remain unchanged, so this metadata layout also works with databases that have already applied them. `pnpm db:migrate` and startup migration execution keep their existing behavior.

Clear the local Docker Postgres database, including Drizzle migration history:

```bash
pnpm db:clear
```

Clear it and re-apply migrations:

```bash
pnpm db:reset
```
