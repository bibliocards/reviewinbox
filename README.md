...
- Classify Reviews with Jev and explore recurring topics, reported severity, trends, and app-version slices in Review analysis.
- Let Luna propose missing English topics for human approval while keeping corrections and catalogue changes under team control.
...
Review analysis is independent of Managed AI Reply Draft quotas. Jev classifies imported Reviews and the complete initial backfill. Luna topic discovery is bounded and separately metered per Organization; commercial discovery quotas are deferred until usage is measured. Set the optional global `TYPESAFE_API_KEY` on the API and worker to enable new classification and discovery jobs. Without it, stored analysis remains readable and sync/reply drafting continue. Luna reuses the configured `AI_PROVIDER`, `AI_MODEL`, `AI_API_KEY`, and optional `AI_BASE_URL` settings; the supplied Compose file passes them to both services.
